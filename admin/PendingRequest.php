...
if($type == "1"){
/* ID */
$qry="SELECT * FROM `db_shop_buy_request` WHERE id=$query and status = 0";
}
elseif($type == "2"){
   /* CNIC */ 
   $qry="SELECT * FROM `db_shop_buy_request` WHERE cus_name LIKE '%".$query."%' and status = 0";
}
elseif($type == "3"){
    /* Mobile */
    $qry="SELECT * FROM `db_shop_buy_request` WHERE cus_mobile=$query";
}
elseif($type == "4"){
   /* Name */ 
   $qry="SELECT * FROM `db_shop_buy_request` WHERE stockID=$query";
}
else{
                ...
                if($customer != "null"){
                    if($customer->num_rows > 0){
                     ...
                     <tr>
                        ...
                        <td>
                            <a href="ProcedBuyRequest.php?ID=<?php echo $row["id"];?>" class="btn-sm btn-success">View</a>
                            <a href="model/deletePendingRequest.php?ID=<?php echo $row["id"];?>" class="btn-sm btn-danger">Delete</a>
                        </td>
                    </tr>
                        <?php
                        }
